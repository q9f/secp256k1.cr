...
describe Secp256k1::Bitcoin do
  ...
  it "can generate a valid bitcoin address" do
    ...
    btc.address.should eq "1PMycacnJaSqwwJqjawXBErnLsZ7RkXUAs"
    ...
    key = Secp256k1::Keypair.new priv
    dog = Secp256k1::Bitcoin::Account.new key, "1e"
    dog.address.should eq "DDh3RAMeWnTWfH6q11uWkF74vMbMxqxa8X"

    # compressed dogecoin address
    comp = Secp256k1::Bitcoin::Account.new key, "1e", true
    comp.address.should eq "DP9Q6DP1GVjUAtcJcaCeR1psedXoC12Jtu"
  end
  ...
  it "can provide the correct wallet import format" do
    ...
    btc.wif.should eq "5HueCGU8rMjxEXxiPuD5BDku4MkFqeZyd4dZ1jvhTVqvbTLvyTJ"
    comp = Secp256k1::Bitcoin::Account.new key, "00", true
    comp.wif.should eq "KwdMAjGmerYanjeui5SHS7JkmpZvVipYvB2LJGU1ZxJwYvP98617"
    ...
    key = Secp256k1::Keypair.new priv
    btc = Secp256k1::Bitcoin::Account.new key
    btc.wif.should eq "5HpHgWkLaovGWySEFpng1XQ6pdG1TzNWR7SrETvfTRVdKHNXZh8"
    comp = Secp256k1::Bitcoin::Account.new key, "00", true
    comp.wif.should eq "KwDidQJHSE67VJ6MWRvbBKAxhD3F48DvqRT6JRqrjd7MHLBjGF7V"
  end
