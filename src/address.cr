...
module Bitcoin
  ...
  def self.address_from_public(pub : String, version = "00")
    ...
    if pub.size === 130 || pub.size === 66
      ...
      # add version byte in front of ripmd-160 hash
      ripe_versioned = "#{version}#{ripe}"

      # perform sha-256 hash on the extended ripmd-160 result
  ...
  def self.address_from_point(p : Secp256k1::EC_Point, version = "00")
    # take the corresponding public key generated with it
    pub = Secp256k1.public_key_compressed_prefix p
    return address_from_public pub, version
  end
  ...
  def self.address_from_private(priv : String, version = "00")
    ...
    return address_from_point p, version
  end
end
...
module Ethereum
  ...
  def self.address_from_public(pub : String)
  ...
  def self.address_from_point(p : Secp256k1::EC_Point)
    # take the corresponding public key generated with it
    pub = Secp256k1.public_key_uncompressed p
    return address_from_public pub
  end
